...
class EXPIRunner:
    ...
    def __init__(self, config):
        ...
        if "t" in self.config["mode"]:
            ...
            self.engine = PredictionEngine(config["engine"], model, self.logger)
        self.save_files()
        setup_seed(777)
    ...
    def run_train(self):
        ...
        test_acts = define_actions("pro1-test", "expi")
        ...
        for act in test_acts:
            ...
            test_loaders[act] = DataLoader(dataset=test_dataset,
        ...
        for epoch in range(start_epoch, self.config["epoch"]):
            ...
            for act in test_acts:
                ...
                act_test_err_avg, act_test_err_all = self.engine.test(
                    ...
                    np.array(self.config["setting"]["eval_frame"]), np.array(self.config["setting"]["dim_used"]),
                    np.array(self.config["setting"]["joint_to_ignore"]),
                    np.array(self.config["setting"]["joint_to_equal"]), test_dataset.time_tsfm, None, act)
                ...
                head_test = np.append(head_test,
                                      [act + "3d200", act + "3d400", act + "3d600", act + "3d800", act + "3d1000"])
    ...
    def run_test(self):
        ...
        for act in test_acts:
            ...
            test_loaders[act] = DataLoader(
                ...
                num_workers=self.config.num_workers,
                pin_memory=True)
            self.logger.info("test data: action {}, shape {}".format(act, test_dataset.all_seqs.shape[0]))
        ...
        for act in test_acts:
            # using transform from train-set
            act_test_err_avg, act_test_err_all = self.engine.test(
                test_loaders[act], self.config["setting"]["input_n"], np.array(self.config["setting"]["eval_frame"]),
                np.array(self.config["setting"]["dim_used"]), np.array(self.config["setting"]["joint_to_ignore"]),
                np.array(self.config["setting"]["joint_to_equal"]), test_dataset.time_tsfm, None, act,
                self.config["save"]["path"]["visualize"] + act if self.config["setting"]["save"] else None)
            # ret_log = np.append(ret_log, test_l)
            # test_err_avg = np.average(test_err_all)
            test_err_avg += act_test_err_avg
            test_err_all += act_test_err_all
    ...
    def run_test_all(self):
        ...
        for act in test_acts:
            # using transform from train-set
            act_test_err_avg, act_test_err_all = self.engine.test(
                test_loaders[act], self.config["setting"]["input_n"], np.arange(self.config["setting"]["output_n"]),
                np.array(self.config["setting"]["dim_used"]), np.array(self.config["setting"]["joint_to_ignore"]),
                np.array(self.config["setting"]["joint_to_equal"]), test_dataset.time_tsfm, test_dataset.scale_tsfm,
                act, self.config["save"]["path"]["visualize"] + act if self.config["setting"]["save"] else None)
            # ret_log = np.append(ret_log, test_l)
            # test_err_avg = np.average(test_err_all)
            len_act = len(test_loaders[act])
            accum_loss_avg += act_test_err_avg * len_act
            accum_loss_all += act_test_err_all * len_act
            ret_log_append = np.append(np.array(act_test_err_avg), act_test_err_all)
            ret_log_append = ret_log_append = np.concatenate((np.array([act]), ret_log_append.astype(np.str)), axis=0)
            ret_log_test.append(ret_log_append)
            N += len_act

        ret_log_append = np.append(np.array(accum_loss_avg / N), accum_loss_all / N)
        ret_log_append = np.concatenate((np.array(["average"]), ret_log_append.astype(np.str)), axis=0)
        ret_log_test.append(ret_log_append)
    ...
    def run_visualize(self):
        ...
        train_dataset = get_dataset(dataset_name, **train_data_config)
        # print("train data shape {}".format(train_dataset.all_seqs.shape[0]))
        self.logger.info("train data shape {}".format(train_dataset.all_seqs.shape[0]))
        ...
        for act in test_acts:
            ...
            for (_, _, _, out_seq) in act_loader:
                ...
                for i in range(n_seq):
                    ...
                    vis_runner.plot_single(vis_seq, self.config["save"]["path"]["visualize"], figure_title,
                                           self.config["setting"]["input_n"])
